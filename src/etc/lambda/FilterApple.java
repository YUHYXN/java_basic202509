...
public class FilterApple {
     ...
     *
     * 리턴 타입 앞에 붙은 <T>는 이 메서드가 제네릭을 사용하는 메서드고, 지금부터 T라는 임시 타입을 선언한다
     * T는 이 메서드가 호출될 때 결정될거야
     */
    // T가 뭐가 될지는 메서드 호출 시점에 결정된다. T는 타입 파라미터
    // 만약 Integer 리스트를 보내면 T는 Integer가 된다.
    // 만약 String 리스트를 보내면 T는 String이 된다.
    // 만약 Apple 리스트를 보내면 T는 Apple이 된다.
